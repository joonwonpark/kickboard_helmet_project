...
![image](apply_model.png)

#### 원본이 들어오고 이후 객체탐지 모델로 헬멧 착용 여부를 확인합니다. 만약에 객체 탐지에 실패한 경우 OpenCV 오픈소스를 이용하여 모자이크 처리한 후 저희 데이터 베이스에 실패 케이스로 저장이 됩니다. 이후 이 이미지들은 모델의 재학습에 사용 될 것입니다.
